...
| **多边形标注** | 点击绘制、顶点编辑、插入/删除顶点、拖拽移动；**开放线自动膨胀**：沿裂纹画一条中心线，双击或 Enter 自动生成带状多边形；**区分光标**：顶点拖拽用十字，整体拖拽用四向箭头；**缩放跟随**：绘制中的顶点在缩放/平移后仍固定在图像正确位置 |
| 多选 | Ctrl+点击多选，批量操作 |
...
| 置信度阈值 | 滑块调节（范围 0.01~0.95），实时过滤低分预测 |
| **IoU 阈值** | 滑块调节（范围 0.05~0.90），控制 NMS 重叠度 |
| 自动标注 | 单张/批量自动标注 |
...
- Active Learning、SAM 集成、旋转框标注、插件系统
...
4. **多边形标注**: 按 `P` 进入多边形模式，逐点击画多边形，双击或点击起点闭合。**裂纹标注**：沿裂缝画一条中心线（2+个点），双击或 Enter 自动膨胀为带状多边形（厚度可在 `canvas.py:POLYGON_THICKNESS` 调整，默认 0.004 约 4 像素半宽）
5. **切换类别**: 选择标注列表下方的类别下拉框
6. **导出**: 点击菜单 `文件 → 导出 YOLO` 或 `Ctrl+E`
...
| `W` | BBox 绘制↔选择模式 |
| `P` | 多边形绘制↔选择模式 |
| `F` | **裂缝模式**：画一条开放线后双击/Enter 自动膨胀为多边形（适合裂纹等细长物体） |
| `S` | 回到选择模式 |
| `H` | 平移模式 |
| `A` / `D` | 上一张 / 下一张 |
| `Ctrl+Z` | 撤销 |
...
每张图片对应 `labels/` 目录下同名 `.txt` 文件（通过 `文件 → 导出 YOLO` 或 `Ctrl+E` 生成）。检测框和分割点可以在同一个文件中混合存储。

**检测格式**（class_id 后 5 个值）：
```
<class_id> <cx> <cy> <width> <height>
```
示例：`0 0.5 0.5 0.3 0.2`

**分割格式**（class_id 后偶数个坐标值）：
```
<class_id> <x1> <y1> <x2> <y2> ... <xn> <yn>
```
示例：`1 0.1 0.1 0.2 0.15 0.3 0.1`

**混合文件示例** — 同一 `.txt` 中同时存在两种格式：
```
0 0.5 0.5 0.3 0.2              ← BBox (类别 0)
1 0.1 0.1 0.2 0.15 0.3 0.1     ← 多边形 (类别 1, 3 个点)
0 0.7 0.8 0.1 0.15             ← 另一个 BBox
```

所有坐标归一化到 [0, 1]。YOLOv8 训练时自动识别每行格式，无需任何额外配置。
